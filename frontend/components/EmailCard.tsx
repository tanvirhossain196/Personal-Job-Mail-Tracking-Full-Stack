        ...
        <div className="px-5 pl-6 py-4">
          <div className="flex items-start justify-between gap-3">
            <div className="flex items-start gap-3 min-w-0">
              <div
                className={cn(
                  "flex h-10 w-10 shrink-0 items-center justify-center rounded-full font-display font-semibold text-sm",
                  activeStyle.bg,
                  activeStyle.text,
                )}
              >
              ...
              <div className="min-w-0">
                <div className="text-[11px] font-mono uppercase tracking-wide text-steel-300">
                  {counterpartLabel}
                </div>
                <div className="text-sm font-medium text-ink truncate">
                  {counterpartName}
                </div>
                <div className="text-xs text-steel-500 font-mono truncate">
                  {counterpartEmail}
                </div>
                <div className="text-sm text-ink mt-1.5 truncate font-medium">
            ...
            <div className="flex flex-col items-end gap-2 shrink-0">
              <span
                className={cn(
                  "inline-flex items-center gap-1.5 rounded-full border px-2.5 py-1 text-[11px] font-medium font-mono uppercase tracking-wide shadow-sm",
                  activeStyle.text,
                  activeStyle.bg,
                  activeStyle.border,
                )}
              ...
              <button
                ...
                className="inline-flex items-center gap-1.5 rounded-md border border-circuit/30 bg-circuit-100 text-circuit-600 px-2.5 py-1.5 text-[11px] font-semibold hover:bg-circuit hover:text-white hover:border-circuit transition-colors"
              >
                <Eye size={12} />
                View mail
              </button>
          ...
          <div className="mt-3.5 pt-3.5 border-t border-steel-100">
            <div className="grid grid-cols-1 sm:grid-cols-[1fr_1fr_1fr_auto] gap-2.5 rounded-md bg-fog-50 border border-steel-100 p-2.5">
              <label className="flex flex-col gap-1">
                ...
                <select
                  ...
                  className="rounded-sm border border-steel-100 bg-white px-2.5 py-1.5 text-xs text-ink outline-none focus:border-circuit focus:ring-1 focus:ring-circuit/30"
                >
              ...
              <label className="flex flex-col gap-1">
                ...
                <input
                  ...
                  placeholder="Company"
                  className="rounded-sm border border-steel-100 bg-white px-2.5 py-1.5 text-xs text-ink outline-none focus:border-circuit focus:ring-1 focus:ring-circuit/30 placeholder:text-steel-300"
                />
              </label>
              <label className="flex flex-col gap-1">
                <span className="text-[9px] font-mono uppercase tracking-widest text-steel-300 px-0.5">
              ...
              <div className="flex items-end">
                <button
                  ...
                  className={cn(
                    "inline-flex items-center justify-center gap-1.5 rounded-sm text-xs font-semibold px-3 py-1.5 transition-colors shrink-0 w-full sm:w-auto",
                    justSaved
          ...
          <div className="flex items-center justify-between mt-2.5 text-[11px] text-steel-300 font-mono">
            <span>{formatDate(reply.receivedAt)}</span>
            {meta?.updatedAt && (
              <span>classified {formatDate(meta.updatedAt)}</span>
            )}
          </div>
        </div>
      </div>
